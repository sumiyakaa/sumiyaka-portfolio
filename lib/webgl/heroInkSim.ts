/* ========================================
   Hero Ink Fluid Simulation — トップFV「墨」レイヤー専用
   ----------------------------------------
   lib/webgl/fluidSim.ts（/about で iOS 実機実証済み）の
   Navier-Stokes パイプラインをそのまま流用した FV 専用モジュール。
   fluidSim.ts 本体には一切手を触れず（/about の見た目・動作は不変）、
   表示シェーダと設定だけを分離した：
   - u_ground : 地色を RGB 個別指定（暖色 #1f1c1c を正確に塗る）
   - u_tint   : 墨の色（暖色白 #f1eaec 系）
   - u_cap    : 墨輝度のハードキャップ（H1 帯の可読性をシェーダで保証）
   ======================================== */

// ---- Shader Sources（fluidSim.ts と同一のパイプライン） ----
...
/* FV 専用の表示シェーダ：
   暖色地に、墨（dye）を暖色白ティントで加算。u_cap が墨輝度の上限
   （H1 帯の背面がこれ以上明るくならないことをシェーダ側で保証する） */
const DSP_FS = `precision highp float;
varying vec2 v_uv;
...
void main() {
  ...
  float ink = min(clamp(d * u_bri, 0.0, 1.0), u_cap);
  gl_FragColor = vec4(u_ground + ink * u_tint, 1.0);
}`;
...
export interface HeroInkSimConfig {
  resolution?: number;
  brightness?: number;
  /** 墨輝度の上限（0-1）。地色に加算される墨の最大量 */
  inkCap?: number;
  /** 地色 RGB（0-1）。既定 #1f1c1c */
  ground?: [number, number, number];
  /** 墨の色 RGB（0-1）。既定 暖色白 */
  inkTint?: [number, number, number];
...
export interface HeroInkSimAPI {
...
export function createHeroInkSim(
  canvas: HTMLCanvasElement,
  config?: HeroInkSimConfig
): HeroInkSimAPI | null {
  const resolution = config?.resolution ?? 0.5;
  const BRI = config?.brightness ?? 0.8;
  const CAP = config?.inkCap ?? 0.36;
  const GROUND = config?.ground ?? [0.1216, 0.1098, 0.1098]; // #1f1c1c
  const TINT = config?.inkTint ?? [0.93, 0.9, 0.895];        // 暖色白の墨
  const VEL_DISS = config?.velocityDissipation ?? 0.984;
  const DYE_DISS = config?.dyeDissipation ?? 0.9945;
  const VORT = config?.vorticity ?? 26.0;
  ...
  const pDsp = mkProg(VS, DSP_FS);
  const uDsp = ulocs(pDsp, ["u_tex", "u_bri", "u_cap", "u_ground", "u_tint"]);
  ...
  function display(): void {
    ...
    gl!.uniform1f(uDsp.u_cap, CAP);
    gl!.uniform3f(uDsp.u_ground, GROUND[0], GROUND[1], GROUND[2]);
    gl!.uniform3f(uDsp.u_tint, TINT[0], TINT[1], TINT[2]);
    gl!.bindFramebuffer(gl!.FRAMEBUFFER, null);
    gl!.viewport(0, 0, W, H);
    gl!.drawArrays(gl!.TRIANGLE_STRIP, 0, 4);
  }
